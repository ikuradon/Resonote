...
describe('Dexie migration metadata', () => {
  it('allows rollback before Dexie-only writes', async () => {
    const store = await createDexieEventStore({
      dbName: 'auftakt-migration-rollback'
    });
  ...
  it('refuses rollback after Dexie-only writes', async () => {
    const store = await createDexieEventStore({
      dbName: 'auftakt-migration-no-rollback'
    });
    await store.recordMigrationState({
      version: 1,
      sourceDbName: 'resonote-events',
...
describe('Dexie retention compaction', () => {
  it('does not compact deletion index, vanish index, or pending publishes before raw old payloads', async () => {
    const store = await createDexieEventStore({
      dbName: 'auftakt-compaction-priority'
    });
