...
export const resonoteTimelineProjection: ProjectionDefinition = defineProjection({
  name: 'resonote.timeline',
  sourceKinds: [1, 1111, 7, 17],
  sorts: [
    { key: 'created_at', pushdownSupported: true },
    { key: RESONOTE_PLAY_POSITION_SORT, pushdownSupported: false }
  ]
});
...
export function createTimelinePlugin(): ResonoteCoordinatorPlugin {
  return {
    ...
    apiVersion: 'v1',
    setup(api) {
      api.registerProjection(resonoteTimelineProjection);
    }
  };
}
