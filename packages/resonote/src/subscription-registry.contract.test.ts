...
describe('@auftakt/resonote shared subscription registry contract', () => {
  it('shares one logical forward request across multiple consumers and tears down only after the final unsubscribe', async () => {
    ...
    const refs = await loadCommentSubscriptionDeps(runtime as unknown as never);
