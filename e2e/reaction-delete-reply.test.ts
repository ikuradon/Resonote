...
test.describe('Reaction flow', () => {
  ...
  test('should display reaction from another user in real-time', async ({ page }) => {
    ...
    // matching buttons elsewhere (e.g., section count badges).
    const commentCard = page
      .locator('article, div')
      .filter({ hasText: 'My comment for reaction' })
      .first();
    // Like button shows count as text after reaction
  ...
  test('should not show reaction buttons when not logged in', async ({ page }) => {
    ...
    await page.goto(TEST_TRACK_URL);
    await page.waitForLoadState('networkidle');
    // Do NOT simulateLogin — testing unauthenticated state
    await broadcastEventsOnAllRelays(page, [comment]);
    ...
    // Reply button is hidden when not logged in (loggedIn check in Shout mode)
    const replyButtons = page.getByRole('button', { name: /^Reply$|^返信$/i });
...
test.describe('Delete flow', () => {
  ...
  test('should delete own comment via ConfirmDialog', async ({ page }) => {
    ...
    const commentEl = page.locator('article, div').filter({ hasText: 'Delete me' }).first();
    await commentEl
  ...
  test('should cancel delete via ConfirmDialog', async ({ page }) => {
    ...
    // Delete is in the More actions menu
    const commentEl = page.locator('article, div').filter({ hasText: 'Keep me' }).first();
    await commentEl
      .getByRole('button', { name: /More actions/i })
      .first()
      .click();
  ...
  test('should not show delete button on other users comments', async ({ page }) => {
    ...
    // Open More actions menu — Delete should not be visible for other user's comment
    const commentEl = page.locator('article, div').filter({ hasText: 'Not yours' }).first();
    await commentEl
      .getByRole('button', { name: /More actions/i })
      .first()
      .click();
    const deleteButtons = page.getByRole('button', { name: /^Delete$|^削除$/i });
...
test.describe('Reply flow', () => {
  ...
  test('should open reply form and post a reply', async ({ page }) => {
    ...
    const commentEl = page.locator('article, div').filter({ hasText: 'Reply to me' }).first();
    const replyButton = commentEl.getByRole('button', { name: /Reply|返信/i }).first();
    await expect(replyButton).toBeVisible({ timeout: 5_000 });
    await replyButton.click();

    // Reply textarea appears inline in the comment card
    const replyTextarea = commentEl.locator('textarea').first();
    await expect(replyTextarea).toBeVisible({ timeout: 5_000 });

    await replyTextarea.fill('This is my reply');

    const sendButton = commentEl.getByRole('button', { name: /^Reply$|^返信$/i }).last();
  ...
  test('should cancel reply form', async ({ page }) => {
    ...
    const commentEl2 = page
      .locator('article, div')
      .filter({ hasText: 'Cancel reply test' })
      .first();
    const replyButton = commentEl2.getByRole('button', { name: /Reply|返信/i }).first();
    await replyButton.click();

    const replyTextarea = commentEl2.locator('textarea').first();
    await expect(replyTextarea).toBeVisible({ timeout: 5_000 });

    const cancelButton = commentEl2.getByRole('button', { name: /Cancel|キャンセル/ }).last();
    await cancelButton.click();
