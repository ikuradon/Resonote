...
const { fetchFollowListSnapshotMock, logErrorMock } = vi.hoisted(() => ({
  fetchFollowListSnapshotMock: vi.fn(),
  logErrorMock: vi.fn()
}));

vi.mock('$shared/auftakt/resonote.js', () => ({
  fetchFollowListSnapshot: fetchFollowListSnapshotMock
}));
...
describe('fetchFollowsCount', () => {
  ...
  it('returns count and pubkeys from p tags', async () => {
    fetchFollowListSnapshotMock.mockResolvedValue({
      tags: [
        ['p', 'pk1'],
        ['p', 'pk2'],
        ['p', 'pk3']
  ...
  it('returns zero when no latest event found', async () => {
    fetchFollowListSnapshotMock.mockResolvedValue(null);
    const result = await fetchFollowsCount(PUBKEY);
  ...
  it('returns zero count on error and logs it', async () => {
    ...
    fetchFollowListSnapshotMock.mockRejectedValue(err);
    const result = await fetchFollowsCount(PUBKEY);
    expect(result.count).toBe(0);
    expect(result.pubkeys).toEqual([]);
  ...
  it('calls fetchLatestEvent with pubkey and FOLLOW_KIND (3)', async () => {
    ...
    expect(fetchFollowListSnapshotMock).toHaveBeenCalledWith(PUBKEY, 3);
  });

  it('returns empty list when event has no tags', async () => {
    fetchFollowListSnapshotMock.mockResolvedValue({ tags: [], content: '', created_at: 1000 });
    const result = await fetchFollowsCount(PUBKEY);
    expect(result.count).toBe(0);
    expect(result.pubkeys).toEqual([]);
  });
