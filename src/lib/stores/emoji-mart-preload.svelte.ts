 ...
 *
 * Call `preloadEmojiMart()` on hover/focus of the emoji button.
...
async function setCachedData(data: unknown): Promise<void> {
  try {
    ...
    // Clear old versions before writing new
...
export function preloadEmojiMart(): void {
  ...
  cached = (async () => {
    ...
    // First time: import raw data, let Picker init process it, then cache
    const rawData = await import('@ikuradon/emoji-kitchen-mart-data').then((m) => m.default);
    // After Picker init, rawData is mutated in-place with search indices.
    // We cache it post-init via a microtask scheduled in getEmojiMartModules().
...
export async function getEmojiMartModules(): Promise<{ data: unknown; Picker: unknown }> {
  ...
  if ('_needsCache' in result && result._needsCache) {
    ...
    // Schedule cache write after Picker constructor processes the data.
    // Picker init mutates data in-place (adds emoji.search fields),
    // so we defer to let the constructor finish before serializing.
