<script lang="ts">
  ...
  import { createTrackInputViewModel } from '$features/content-resolution/ui/track-input-view-model.svelte.js';
  import { t } from '$shared/i18n/t.js';

  const vm = createTrackInputViewModel({ navigate: goto });
</script>
...
>
  <div class="flex gap-3">
    <input
      ...
      style="--placeholder-opacity: {vm.placeholderVisible ? 1 : 0}"
    />
    <button
      type="submit"
  ...
  {#if vm.error}
    <p role="alert" class="animate-fade-in text-sm text-error">{vm.error}</p>
  {/if}
</form>
