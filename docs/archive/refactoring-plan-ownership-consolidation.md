...
> `shared` を public API、`lib` を internal/compat として最終収束させる。その際、notifications / comments / profile / relays に残る coordinator ロジックを feature/shared に寄せ、stateful 実装の ownership を一本化する。
...
これらに stateful 実装を残すなら、temporary internal なのか compat なのかを明記する。理由のない滞留は認めない。
...
7. `lib/stores` / `lib/nostr` に残るものが `internal` か `compat` か説明できる。
8. lint がこの分類を守れる。
...
3. `shared` の public/internal/compat/pure-helper 分類を固定する
4. lint hardening
5. legacy cleanup
...
- `lib/stores/*` / `lib/nostr/*` のうち compat と internal を区別する。
