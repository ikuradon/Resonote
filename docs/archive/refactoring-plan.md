...
- UI component から `castSigned`, `getRelaySession`, `getEventsDB`, `fetch('/api/...')` の直接依存が消えている
...
- `ui -> infra`
- `route/layout -> relay-session / IndexedDB / raw fetch`
...
- `getRelaySession`, `castSigned`, `getEventsDB`
- raw `fetch('/api/...')`
