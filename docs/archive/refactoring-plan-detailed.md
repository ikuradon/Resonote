...
- UI component から `castSigned`, `getRelaySession`, `getEventsDB`, raw `fetch('/api/...')`, raw browser messaging が消えている
...
- `ui -> infra`
- `route/layout -> relay-session / IndexedDB / raw fetch / raw browser messaging`
...
- 禁止ルールを少なくとも次にかける
  ...
  - `web/routes/** -> relay-session`, `event-db`, `publish-signed`
