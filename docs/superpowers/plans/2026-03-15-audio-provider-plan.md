...
const providers: ContentProvider[] = [
  spotify, youtube, vimeo, netflix, primeVideo, disneyPlus,
  appleMusic, soundcloud, fountainFm, abema, tver, uNext, mixcloud, spreaker,
...
<script lang="ts">
  ...
  async function resolve(url: string) {
    try {
      ...
      if (data.error) {
        status = 'error';
        errorMessage = data.error === 'rss_not_found'
...
```typescript
return {
  get comments() { return visibleComments; },
  get reactionIndex() { return reactionIndex; },
...
describe('parseDTagEvent', () => {
  ...
  it('returns null for event without podcast:item:guid tag', () => {
    const event = {
      kind: 39701,
      tags: [['d', 'example.com/feed.xml'], ['k', 'podcast:guid']],
    };
...
export interface ResolveApiResponse {
  ...
  episode?: { guid: string; title: string; enclosureUrl: string; duration: number; publishedAt: number };
    ...
    <div class="flex flex-col gap-1">
      {#each episodes ?? [] as ep (ep.guid)}
        ...
        >
          ...
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5 shrink-0 text-zinc-500">
            <path d="M8 5v14l11-7z"/>
          </svg>
        </button>
      {/each}
...
async function handleFeedUrl(feedUrl: string, privkey: Uint8Array) {
  try {
    ...
    const signedEvents = [];
    // Sign feed bookmark
    signedEvents.push(signBookmarkEvent(privkey, {
...
function parseItem(itemXml: string): { guid: string; title: string; enclosureUrl: string; duration: number; publishedAt: number } | null {
