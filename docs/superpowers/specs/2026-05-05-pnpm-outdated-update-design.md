...
`pnpm outdated --format table` の結果、root `package.json` と `pnpm-lock.yaml` の wanted/current に差分がある依存と、target version まで未更新の direct dependency が複数ある。今回の目的は、2026-05-05 時点の `pnpm outdated` snapshot に出た direct dependency を target version へ更新すること。ただし、SvelteKit/Vite/TypeScript/Wrangler/esbuild など build・typecheck・runtime に影響しやすい依存が含まれるため、changelog 確認と検証を省略しない。
...
- 2026-05-05 時点の `pnpm outdated` に表示された direct dependency を、本 spec の target version まで更新する。
- 各 dependency について、現在版から target version までの changelog/release notes を確認する。
- 作業開始時に target version、Node/pnpm version、baseline gate を記録する。
...
`pnpm outdated --format json` の結果を作業開始時の snapshot とし、今回の target version は本 spec の表に固定する。更新コマンドでは `package@latest` ではなく `package@<target version>` を指定する。

例:

```bash
pnpm add -D typescript@6.0.3 vite@8.0.10
pnpm add hono@4.12.17 zod@4.4.3
```
...
2026-05-05 時点の `pnpm outdated --format table` で確認した対象は次の通り。`Target` は今回固定する exact target であり、作業中に npm の `latest` が動いても変更しない。

| Package                            | Current  | Wanted    | Target    | 区分    | Risk | Batch |
| ---------------------------------- | -------- | --------- | --------- | ------- | ---- | ----- |
...
このバッチは lockfile-only に見えるが、manifest が既に major/minor update を要求している依存を実際に解決するため、通常の依存更新と同じく changelog 確認を必須にする。
...
`typescript`, `@codecov/vite-plugin`, `eslint-plugin-simple-import-sort`, `nano-staged` は major boundary を跨ぐ。`vite`, `hono`, `@sveltejs/kit`, `typescript-eslint` は wanted と target が異なるため、このバッチでは「wanted まで同期する」か「後続の専門バッチで target まで上げる」かを作業ログで明示する。失敗時はこのバッチ内でも dependency 単位に分解する。
...
TypeScript 6 は high-risk とし、deprecated option、`import ... asserts` から import attributes への移行、`tsc file.ts` 実行時の扱い、compiler option、module resolution、DOM lib、strictness 変更を確認する。検証は `pnpm run check`, `pnpm run lint`, `pnpm run test:packages` を必須にする。
...
Vite 8 では Rolldown/Oxc 統合により、production build、chunking、CSS minification、plugin hooks、`rollupOptions` / `esbuild` config の挙動差分を重点確認する。Vite 公式は複雑な project では migration guide 確認と段階的移行を推奨しているため、失敗時は Vite/SvelteKit/Svelte/esbuild を個別に切り分ける。
...
CI で browser cache を使っている場合、cache key に `@playwright/test` version が含まれているか確認する。local で `install --with-deps` が権限や platform の都合で実行できない場合は、理由と代替検証を記録する。
...
各 dependency について、現在版から target version までの範囲で以下を確認する。
...
各 install 後に peer dependency と top-level 解決状態を確認する。

```bash
pnpm install
pnpm explain peer-requirements
pnpm list --depth 0
```

`pnpm install` の peer warning と engine warning は作業ログに残す。特に以下の組み合わせは peer/config compatibility を確認する。
...
- `pnpm outdated --format table` を再実行し、今回固定した target version まで direct dependency が更新されていることを確認する。
- 作業中に npm の `latest` が進んだ場合、その差分は今回 scope 外として記録する。
- 各 dependency の changelog 確認結果が残っている。
- `pnpm explain peer-requirements` と `pnpm list --depth 0` の確認結果が残っている。
- `package.json` と `pnpm-lock.yaml` が target 更新を反映している。
