...
作業時は repo の `packageManager` field に記載された pnpm version を使う。local の `pnpm --version` が `packageManager` と異なる場合は、Corepack で repo-pinned version に合わせるか、差分を記録して作業を止める。package manager version の変更は本 spec の scope に含めない。
...
```bash
npm diff --diff='@scure/base@2.0.0' --diff='@scure/base@2.2.0'
npm diff --diff='@konemono/nostr-login@1.15.2' --diff='@konemono/nostr-login@1.15.7'
npm diff --diff-name-only --diff='@scure/base@2.0.0' --diff='@scure/base@2.2.0'
npm diff --diff-name-only --diff='@konemono/nostr-login@1.15.2' --diff='@konemono/nostr-login@1.15.7'
```
...
```bash
pnpm install
pnpm install 2>&1 | tee logs/pnpm-install-batch-N.log

# pnpm 11+ only
pnpm peers check

# pnpm 10 以下では `pnpm peers check` を実行せず install log を確認する
pnpm list --depth 0
pnpm run check
```
