...
1. relay-session-like reconnect resilience and automatic REQ optimization, including
...
4. NIP compliance through an explicit current matrix with owners, proof anchors,
   and clear `implemented`, `internal`, `interop`, `unsupported-by-design`, and
...
- adding full NDK-interoperable models
...
- docs do not claim strict final completion where only scoped completion is
  proven
- NIP matrix wording distinguishes implemented, internal, interop,
