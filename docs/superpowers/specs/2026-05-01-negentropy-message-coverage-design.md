...
- uppercase hex input を accept し、decode 結果を lowercase hex IDs に normalize すること
...
- skip range の後ろに ID-list frame がある message では、skip range は parse / consume するが
  event ID 生成対象にはせず、ID-list frame の IDs だけを返すこと
...
verification / relay repair の contract tests も同時に実行し、internal helper の直接 import
や追加 test file による regression がないことを確認する。
