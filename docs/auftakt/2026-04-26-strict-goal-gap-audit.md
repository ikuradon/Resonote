...
| strfry-like local-first event processing     | `Partial`          | `EventCoordinator`, Dexie materialization, quarantine, hot index, deletion handling, replaceable heads, relay hints, high-level local store helpers, and strict closure guards exist.                                                                                                       | The browser store is not a full local relay database abstraction, and raw session usage remains inside coordinator-owned runtime helpers. Allowed internal transport zones need explicit audit classification.                                                                           | Keep local store and coordinator mediation regression gates active.                        |
