...
### 14.1 Foundation 由来ギャップのスコープ行列

| 区分        | 対象                                                          | 現時点の扱い                                                                                |
| :---------- | :------------------------------------------------------------ | :------------------------------------------------------------------------------------------ |
| **include** | `requestKey` / replay canonicalization                        | 今回の実装波に含める。proof と status 文言もこの前提で管理する。                            |
| **include** | typed observations (`RelayObservation`, `SessionObservation`) | 今回の実装波に含める。relay/session recovery の正典語彙として扱う。                         |
| **include** | `ReadSettlement` semantics                                    | 今回の実装波に含める。consumer 露出だけでなく内部 ad-hoc フラグ解消まで追う。               |
| **include** | reconcile / offline publish semantics                         | 今回の実装波に含める。reconcile reason/state と offline publish queue を proof 対象に残す。 |
| **include** | negentropy internal sync / repair                             | 今回の実装波に含める。ただし app-facing API には raw `NEG-*` を露出しない。                 |
| **include** | remaining targeted consumer cutover                           | 今回の実装波に含める。`--report consumers` は façade 完了とは別に残存 cutover を追跡する。  |
| **exclude** | unrelated UI refactors                                        | 本仕様のギャップ閉鎖対象外。consumer cutover に必要な最小変更のみを扱う。                   |
| **exclude** | raw app-facing negentropy APIs                                | 本仕様の対象外。negentropy は internal-only sync/repair capability として閉じ込める。       |

| 項目                           | ステータス                    | 現状アンカー (Current repo reality anchor)                                                            | 既知のギャップ (Known gap)                                                                                                                                                                    |
| :----------------------------- | :---------------------------- | :---------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **App-facing façade**          | `implemented`                 | `src/shared/auftakt/resonote.ts`                                                                      | façade は唯一の app-facing import point として維持され、remaining targeted consumer cutover も完了した。`pnpm run check:auftakt-migration -- --report consumers` は `PASS` を返す。           |
| **ReadSettlement**             | `implemented`                 | `packages/core/src/index.ts` / `src/shared/auftakt/resonote.ts`                                       | canonical `ReadSettlement` contract への cutover が完了し、verification は `packages/core/src/read-settlement.contract.test.ts` と `src/shared/nostr/cached-query.test.ts` に固定されている。 |
...
| **negentropy**                 | `implemented (internal-only)` | `packages/adapter-relay/src/index.ts` / `packages/resonote/src/runtime.ts`                            | raw `NEG-*` は adapter/runtime 層に閉じ込め、unsupported / decode failure / transport failure 時は canonical backward repair へ fallback する。app-facing façade には露出しない。             |
...
キーワード追跡: `ReadSettlement`, `relay lifecycle`, `recovery`, `tombstone`, `negentropy`, `publish`, `session`, `consumer cutover`
