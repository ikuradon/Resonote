...
- **NIP-04** は broad public support
  ...
  として記述する。
- **NIP-19** は標準 `npub` / `nprofile` / `note` / `nevent` の
  encode/decode/route を public claim とし、`ncontent` は Resonote-specific
  extension として分離して記述する。
...
Auftakt の 7 つの主要目標に対する現在の達成状況を以下に定義する。判定は
`Satisfied` (達成), `Partial` (部分達成), `Not Yet Proven` (未証明) の 3
値で行われる。
