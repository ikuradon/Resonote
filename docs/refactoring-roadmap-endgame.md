...
| Residual 4: Compat Delete / Lint Hardening | `done` | `lib/stores` を削除し、再流入を lint で止めた | `src/lib/stores/*`, `eslint.config.js` |
...
4. `Residual 4` を実施する
   - compat file を消し、lint 例外を 0 にする
...
- [x] `lib/i18n/t.ts` を compat wrapper にせず削除した
...
#### Residual 4: Compat Delete / Lint Hardening
...
| Residual Checkpoint D | `done` | `rg -n '\$lib/stores/.*svelte|../stores/.*svelte' src --glob '!**/*.test.*'` が 0 件 |
...
- compatibility layer は、減るコード量が増えるコード量を上回るときだけ作る
...
- 2026-03-21: Residual 4-5 を完了し、`src/lib/stores` compat を削除、lint 例外を 0 にし、Residual Zero Achieved まで文書を更新した
