...
- **Nostr**: rx-nostr + @rx-nostr/crypto (verifier/signer) + @ikuradon/auftakt (event store)
...
- `src/shared/nostr/store.ts`: auftakt EventStore シングルトン (`getStore()`, `initStore()`, `disposeStore()`, `fetchLatest()`)。connectStore で rx-nostr のイベントを自動キャッシュ。getSync / fetchById で IDB + リレーの2段フォールバック取得
...
- 削除済み: `event-db.ts`, `cached-query.svelte.ts`, `gateway.ts` (auftakt の store.ts に統合)
...
- shared nostr テスト:
  - `src/shared/nostr/store.test.ts` (auftakt store singleton, initStore, fetchLatest)
...
- auftakt の `store.fetchById()` は in-flight dedup + negative cache + deletedIds チェックを内蔵。`cachedFetchById` は削除済み
