...
- `src/shared/nostr/client.ts`: Singleton `getRelaySession()` with Auftakt relay verifier
...
- `src/shared/nostr/publish-signed.ts`: Pre-signed event publish via `relaySession.cast()` + pending queue
...
- クライアントが `relaySession.cast()` で publish (署名済みイベントは再署名されずそのまま通る)
...
- interop retirement の完全性は `pnpm run check:auftakt-migration -- --proof` で検証可能。この proof が唯一の authoritative machine-checkable gate
...
- `client.ts` の `disposeRelaySession()` でテスト間のシングルトンリセット
