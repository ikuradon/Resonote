...
- E2E テストのリレー: アプリのバンドル済み `DEFAULT_RELAYS` (実 URL) を MockPool に登録。バンドルの `var Tsunagiya` は `window.Tsunagiya` に書き換えて inject (Playwright `addInitScript` は関数スコープで wrap するため `var` が `window` に昇格しない)
